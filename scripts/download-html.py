...
class HTMLDownloader:
    ...
    def __init__(
        self, url: str, output: str, with_all: bool = False, with_images: bool = False
    ) -> None:
    ...
    def _fix_html_for_offline(
        self, html_content: str, resources_dir: Optional[Path]
    ) -> str:
        ...
        for pattern in patterns:
            html_content = re.sub(
                pattern, fix_relative_url, html_content, flags=re.IGNORECASE | re.DOTALL
            )
...
INSTALLATION:
  ...
  See the script header for detailed installation instructions.
    ...
    parser.add_argument(
        "-u", "--url", required=True, help="URL of the page to download"
    )
    ...
    parser.add_argument(
        "--with-images", action="store_true", help="Download HTML and images only"
    )
    ...
    try:
        ...
        if "playwright" in str(e).lower():
            ...
            print(
                "1. Global: pipx install playwright && playwright install chromium && pip install playwright"
            )
        ...
        if "file not found" in error_msg or "winerror 2" in error_msg:
            ...
            print(
                "Please install the PyPI version instead. See script header for instructions."
            )
        else:
            print(f"Error: {e}")
        sys.exit(1)
