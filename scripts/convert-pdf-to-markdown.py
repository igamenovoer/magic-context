...
def check_markitdown_available() -> bool:
    ...
    try:
        result = subprocess.run(
            ["markitdown", "--help"], capture_output=True, text=True, timeout=10
        )
...
def get_markitdown_command(auto_yes: bool = False) -> Tuple[List[str], str]:
    ...
    if check_uvx_available():
        if auto_yes:
            print(
                "markitdown not found directly, using 'uvx markitdown' (may install if not cached)"
            )
            return (["uvx", "markitdown"], "uvx markitdown")
        else:
            print("markitdown is not directly available, but 'uvx' is detected.")
            print(
                "This can run 'uvx markitdown' which may automatically install markitdown if needed."
            )
...
def confirm_overwrite(path: Path, auto_yes: bool = False) -> bool:
    ...
    if path.name == "uvx-install-confirmation":
        ...
        while True:
            response = (
                input(
                    "Do you want to use 'uvx markitdown'? (This may install markitdown automatically) (y/n): "
                )
                .lower()
                .strip()
            )
...
def convert_pdf_with_python_api(
    pdf_file: Path,
    output_dir: Path,
    use_llm: bool = False,
    azure_endpoint: Optional[str] = None,
...
def create_readme(
    ...
    with open(readme_path, "w", encoding="utf-8") as f:
        ...
        f.write(
            "This directory contains the results of PDF to Markdown conversion.\n\n"
        )
        ...
        if images_dir and images_dir.exists():
            ...
            f.write(
                "- Images were extracted from the PDF and organized in this directory\n"
            )
        ...
        f.write(
            "- For best results with complex layouts, consider using Azure Document Intelligence\n"
        )
...
Features:
  ...
  - Automatic image organization and path correction
    ...
    output_dir, output_markdown_file, markdown_filename = determine_output_paths(
        args.output
    )
    ...
    if output_markdown_file.exists() and not confirm_overwrite(
        output_markdown_file, args.yes
    ):
    ...
    if args.use_llm and not os.getenv("OPENAI_API_KEY"):
        print(
            "Warning: OPENAI_API_KEY environment variable not set. LLM features disabled."
        )
        args.use_llm = False
    ...
    for i, pdf_file in enumerate(pdf_files):
        ...
        if args.extract_images:
            file_images_dir = (
                images_dir / pdf_file.stem if len(pdf_files) > 1 else images_dir
            )
        ...
        if markdown_content:
            ...
            if extracted_images:
                markdown_content = append_extracted_images(
    ...
    create_readme(
        output_dir,
        converted_files,
        images_dir if args.extract_images else None,
        processing_info,
    )
    ...
    if args.extract_images and images_dir.exists():
        image_count = len(list(images_dir.glob("**/*.png"))) + len(
