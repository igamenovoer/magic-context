...
def convert_html_to_markdown(
    html_file: Path, output_dir: Path, temp_dir: Path, markitdown_cmd: List[str]
) -> Optional[Path]:
...
def fix_image_paths(
    temp_markdown_file: Path, image_dirs: List[Path], output_markdown_file: Path
) -> Path:
...
def create_readme(
    output_dir: Path, converted_files: List[Path], images_dir: Optional[Path]
):
  ...
  %(prog)s -i ./source -o ./result.md --yes
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for i, html_file in enumerate(html_files):
            temp_converted_file = convert_html_to_markdown(
                html_file, output_dir, temp_path, markitdown_cmd
            )
            if temp_converted_file:
                ...
                if image_dirs and not args.keep_original_images:
                    final_file = fix_image_paths(
                        temp_converted_file, image_dirs, final_output_file
                    )
