...
def main() -> int:
    ...
    explicit_inputs = [coerce_arg_to_glob(t) for t in args.targets if str(t).strip()]
    explicit_inputs = [t for t in explicit_inputs if t]
...
def _service_readme(
    *,
    ...
    return textwrap.dedent(
        ...
        - `{service_name}/docview.yml` stores the default scan patterns and gitignore policy (`scan.respect_gitignore`, `scan.include_globs`, `scan.force_globs`, `scan.exclude_globs`).
        - `{service_name}/mkdocs.yml` is generated only if missing (existing configs are preserved).
        - {bind_note}
        - Mermaid diagrams are supported via `pymdownx.superfences` and Material’s Mermaid integration.
