...
# Preference on Linux/macOS: bunx (if available) > npx
runner_kind=""
runner=""
...
if lr_has_cmd bun; then
  runner_kind="bun"
  runner="bunx"
elif lr_has_cmd node && lr_has_cmd npx; then
  runner_kind="node"
  runner="npx"
else
  lr_die "No suitable runner found. Install bun or Node.js first."
fi
