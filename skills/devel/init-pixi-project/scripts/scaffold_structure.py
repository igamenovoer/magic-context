...
def generate_readme_content(dir_path, description):
    ...
    try:
        ...
        if items:
            content.append("\n## Existing Content")
            content.append(
                "The following files and directories were present when this README was generated:\n"
            )
            for item in items:
...
def main():
    ...
    parser.add_argument(
        "--package-name",
        help="Name of the python package (default: project_root name)",
        default=None,
    )
    ...
    package_name = (
        ...
        else project_name.replace("-", "_").lower()
    )
