...
Goal: list session names and their stored `thread_id` metadata for a given workspace.
...
Print machine-readable JSON, which is the default:
...
- Each alias entry may include `last_model`, which becomes the default `--model` when resuming that session unless explicitly overridden.
