...
if has_cmd codex && [ "$force" -ne 1 ]; then
  log "codex is already available on PATH. Use --force to reinstall."
  exit 0
fi
...
ensure_node_bins_on_path
ensure_user_bins_on_path
has_cmd codex || die "Installation finished, but 'codex' is still not on PATH. Check your global package bin directory."
log "Installed Codex CLI: $(command -v codex)"