# Mermaid `sequenceDiagram` Styling Guide
...
- Keep arrow text concise (ideally under ~40 characters).
...
- For repeated long prefixes, shorten with an ID or alias:
  - Use `TO as torch.ops<br/>spqr_cuda` once, then `TO->>CXX: spqr_mul_fused(...)`.
...
- Avoid deep nesting; if you need more than 2 nested blocks, split the diagram.
